...
public interface WalletLogDao {
    @Query("SELECT * FROM wallet_log_table WHERE wallet_log_owner =:owner AND wallet_log_access=1")
    LiveData<List<WalletLogEntity>> getWalletLog(String owner);

    @Query("SELECT * FROM wallet_log_table WHERE wallet_log_id =:id AND wallet_log_access=1 AND wallet_log_owner =:owner limit 1")
